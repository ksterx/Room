...
class EnvWrapper(ABC):
    ...
    def __getattr__(self, key: str) -> Any:
        ...
        raise AttributeError(
            "Wrapped environment ({}) does not have attribute '{}'".format(self._env.__class__.__name__, key)
        )
...
class IsaacGymPreview2EnvWrapper(EnvWrapper):
    def __init__(self, env: Any) -> None:
...
class IsaacGymPreview3EnvWrapper(EnvWrapper):
    def __init__(self, env: Any) -> None:
...
class OmniverseIsaacGymEnvWrapper(EnvWrapper):
    def __init__(self, env: Any) -> None:
...
class GymEnvWrapper(EnvWrapper):
    def __init__(self, env: Any) -> None:
        ...
        if self._drepecated_api:
            notice.warning("Using a deprecated version of OpenAI Gym's API: {}".format(gym.__version__))
...
class DeepMindEnvWrapper(EnvWrapper):
    def __init__(self, env: Any) -> None:
...
class RobosuiteEnvWrapper(EnvWrapper):
    def __init__(self, env: Any) -> None:
    ...
    def _spec_to_space(self, spec: Any) -> gym.Space:
        ...
        if type(spec) is tuple:
            return gym.spaces.Box(shape=spec[0].shape, dtype=np.float32, low=spec[0], high=spec[1])
...
def wrap_env(env: Any, wrapper: str = "auto", verbose: bool = True) -> EnvWrapper:
    ...
    if verbose:
        notice.info(
            "Environment class: {}".format(
                ", ".join([str(base).replace("<class '", "").replace("'>", "") for base in env.__class__.__bases__])
            )
        )
    if wrapper == "auto":
        base_classes = [str(base) for base in env.__class__.__bases__]
        if (
            "<class 'omni.isaac.gym.vec_env.vec_env_base.VecEnvBase'>" in base_classes
            or "<class 'omni.isaac.gym.vec_env.vec_env_mt.VecEnvMT'>" in base_classes
        ):
            if verbose:
                notice.info("Environment wrapper: Omniverse Isaac Gym")
            return OmniverseIsaacGymEnvWrapper(env)
        elif isinstance(env, gym.core.Env) or isinstance(env, gym.core.EnvWrapper):
            if verbose:
                notice.info("Environment wrapper: Gym")
            return GymEnvWrapper(env)
        elif isinstance(env, gymnasium.core.Env) or isinstance(env, gymnasium.core.EnvWrapper):
            if verbose:
                notice.info("Environment wrapper: Gymnasium")
            return GymnasiumEnvWrapper(env)
        elif "<class 'dm_env._environment.Environment'>" in base_classes:
            if verbose:
                notice.info("Environment wrapper: DeepMind")
            return DeepMindEnvWrapper(env)
        elif "<class 'robosuite.environments." in base_classes[0]:
            if verbose:
                notice.info("Environment wrapper: Robosuite")
            return RobosuiteEnvWrapper(env)
        elif "<class 'rlgpu.tasks.base.vec_task.VecTask'>" in base_classes:
            if verbose:
                notice.info("Environment wrapper: Isaac Gym (preview 2)")
            return IsaacGymPreview2EnvWrapper(env)
        if verbose:
            notice.info("Environment wrapper: Isaac Gym (preview 3/4)")
        return IsaacGymPreview3EnvWrapper(env)  # preview 4 is the same as 3
    elif wrapper == "gym":
        if verbose:
            notice.info("Environment wrapper: Gym")
        return GymEnvWrapper(env)
        ...
        if verbose:
            notice.info("Environment wrapper: Isaac Gym (preview 3)")
        return IsaacGymPreview3EnvWrapper(env)
    elif wrapper == "isaacgym-preview4":
        if verbose:
            notice.info("Environment wrapper: Isaac Gym (preview 4)")
        return IsaacGymPreview3EnvWrapper(env)  # preview 4 is the same as 3
